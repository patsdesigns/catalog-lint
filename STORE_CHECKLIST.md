...
| Privacy policy URL | Done | https://catalog-lint.onrender.com/privacy (`app/routes/privacy.jsx`); update it in the same change whenever what TidyUp stores changes. It says what TidyUp stores: product data findings, fix logs, settings and support form messages; no customer data. Support form messages (name, email address, message) are also emailed to hello@patsdesigns.com through Resend. Required for the listing. Add the weekly email address when that feature returns. |
...
| Pricing in the listing | In progress | Pricing method: **Manual pricing (legacy)**. Never click "Get started" on the App Pricing banner: once an app opts in to App Pricing, Shopify refuses the Billing API charges the Plans page makes. Four public plans that must match `app/lib/plans.js`: Dust Off free (20 products), Quick Clean $10 monthly, Deep Clean $20 monthly, Deep Clean Early Bird $10 monthly for the first 50 paying stores (not available on development stores). No trial. **Remove the Early Bird plan card from the listing once the 50 seats are taken.** |
...
| Environment | Done on Render | `SHOPIFY_API_KEY`, `SHOPIFY_API_SECRET`, `SHOPIFY_APP_URL`, `SCOPES`, `DATABASE_URL=file:/data/tidyup.sqlite`, `NODE_ENV=production`, `BILLING_TEST=false`, `RESEND_API_KEY` (support emails; needed), optionally `EMAIL_FROM` (once patsdesigns.com is verified in Resend), `SUPPORT_EMAIL` (default hello@patsdesigns.com) and `SUPPORT_WEBHOOK_URL`. `CRON_SECRET` is only for the weekly email, not needed while it is in no plan. |
