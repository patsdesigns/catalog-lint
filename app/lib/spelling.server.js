...
export function textFields(p) {
  const fields = [
    { field: "Title", key: "title", text: p.title || "" },
    { field: "Description", key: "descriptionHtml", text: stripHtml(p.descriptionHtml) },
    { field: "SEO title", key: "seoTitle", text: p.seoTitle || "" },
...
// Skips short words, capitalized words (brands, names), and mixed case codes.
export function findMisspellings(fields, ctx) {
  const seen = new Set();
  const out = [];
  for (const { field, key, text } of fields) {
    for (const match of text.matchAll(WORD_RE)) {
      ...
      if (ctx.speller.correct(word)) continue;
      seen.add(lower);
      const suggestion = ctx.speller.suggest(word)[0] || null;
