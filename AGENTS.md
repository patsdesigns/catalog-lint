...
- Pages use Polaris web components only, sentence-case headings, verbs on buttons, no exclamation marks. Color comes only from Polaris tones (critical, warning, caution, success, info, neutral), applied by meaning: severity, clean, trend. Never a color of our own; the app is to pass Shopify's review. The two custom-styled elements are the display-size summary numbers and the trend bars on the home page, both taking their color from the Polaris text around them.
